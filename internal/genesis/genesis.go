...
import (
	...
	"github.com/aeltai/hangar-genesis/internal/generator/appcollection"
	"github.com/aeltai/hangar-genesis/internal/generator/kdmimages"
	"github.com/aeltai/hangar-genesis/internal/generator/listgenerator"
...
type genesisOpts struct {
	...
	// Optional Hauler manifest export
	exportHauler bool
	haulerOutput string
}
...
type generateListConfig struct {
	...
	Export                     *exportConfig       `yaml:"export"`                     // Optional export options
}

type exportConfig struct {
	Hauler       bool   `yaml:"hauler"`
	HaulerOutput string `yaml:"haulerOutput"`
}
...
func (cc *genesisCmd) loadConfigFile() error {
	...
	if config.Export != nil {
		cc.exportHauler = config.Export.Hauler
		cc.haulerOutput = config.Export.HaulerOutput
	}
...
func (cc *genesisCmd) finish() error {
	...
	if cc.exportHauler {
		out := cc.haulerOutput
		if out == "" {
			out = cc.rancherVersion + "-hauler.yaml"
		}
		if err := export.WriteHaulerManifest(imagesLinuxList, cc.rancherVersion, out); err != nil {
			return fmt.Errorf("write hauler manifest: %w", err)
		}
		logrus.Infof("Exported Hauler manifest to %v", out)
	}
	if cc.interactive {
